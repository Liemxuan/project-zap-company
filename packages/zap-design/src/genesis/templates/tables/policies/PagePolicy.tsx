...
import { usePolicies } from '@/hooks/policy/use-policies';
import { Policy } from '@/services/policy/policy.model';
import { getPolicyColumns } from './components/columns';
import { getPolicyFilterGroups } from './components/filters';
import { PolicyInspector } from './components/inspector';

/**
 * PagePolicyTemplate
 * Standardized Administrative Module for Policies & Terms
 */
export default function PagePolicyTemplate() {
    ...
    // --- State ---
    const [selectedItem, setSelectedItem] = useState<Policy | null>(null);
    ...
    const handleRowClick = (item: any) => {
        setSelectedItem(item as Policy);
        setInspectorState('expanded');
    };

    // --- Mapped Data & Columns ---
    const columns = useMemo(() => getPolicyColumns(handleEdit, handleDelete), [handleEdit, handleDelete]);
    const filterGroups = useMemo(() => getPolicyFilterGroups(apiFilters), [apiFilters]);

    const tableComponent = (
        <ListTable
            initialItems={policies as any}
            isLoading={isLoading}
            ...
            <div className="flex-1 flex flex-col min-w-0 bg-layer-base/50 relative">
                <div className="h-14 border-b border-border bg-layer-base flex items-center px-6 justify-between shrink-0 shadow-sm z-10 relative">
                    <div className="flex items-center text-xs gap-1 font-dev text-transform-tertiary">
                        <span className="opacity-50 uppercase tracking-widest">Legal</span>
                        <Icon name="chevron_right" size={14} className="opacity-30" />
                        <span className="font-bold text-on-surface uppercase tracking-widest">Policies</span>
                    </div>
                </div>
        ...
        return (
            <div className="flex h-screen w-full bg-layer-canvas overflow-hidden font-sans">
                ...
                <div className="flex-1 flex flex-col min-w-0 bg-transparent relative">
                    <ThemeHeader title="Policies & Terms" badge={null} />
    ...
    return (
        <ComponentSandboxTemplate
            componentName="Policies-Management"
            tier="L6 LAYOUT"
            status="Verified"
            filePath="src/genesis/templates/tables/policies/PagePolicy.tsx"
            importPath="@/genesis/templates/tables/policies/PagePolicy"
        ...
        >
            <div className="w-full flex-1 flex items-center justify-center pt-8">
                <CanvasDesktop
                    title="Policies // Legal Module"
                    fullScreenHref={`/design/${activeTheme}/organisms/policies?fullscreen=true`}
                >
                    {layoutContent}
                </CanvasDesktop>
            </div>
