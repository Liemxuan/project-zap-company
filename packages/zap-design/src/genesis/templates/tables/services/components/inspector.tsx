                    ...
                    <div className="space-y-4 animate-in fade-in slide-in-from-right-2 duration-300">
                        <div className="p-4 rounded-xl bg-primary/5 border border-primary/10 space-y-3">
                            <div className="flex justify-between items-start">
                                <span className="text-[11px] font-mono opacity-50 uppercase tracking-tighter">{selectedService.serial_id}</span>
                                <Pill variant={selectedService.status_color as any || 'success'}>{selectedService.status_text}</Pill>
                            </div>
                            ...
                            <div className="pt-2 border-t border-primary/10 grid grid-cols-2 gap-4">
                                <div>
                                    <span className="block text-[10px] uppercase opacity-40 font-bold mb-1">{t.label_price}</span>
                                    <span className="text-sm font-mono font-bold">
                                        {selectedService.price.toLocaleString()} VND
                                    </span>
                                </div>
                                <div>
                                    <span className="block text-[10px] uppercase opacity-40 font-bold mb-1">{t.label_category}</span>
                                    <span className="text-sm font-bold">{selectedService.category}</span>
                                </div>
                            </div>
                        </div>
                    </div>
