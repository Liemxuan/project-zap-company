            ...
            <div className="flex-1 overflow-y-auto p-6 space-y-8">
                ...
                {selectedService ? (
                    <div className="space-y-4 animate-in fade-in slide-in-from-right-2 duration-300">
                        <div className="p-4 rounded-xl bg-primary/5 border border-primary/10 space-y-3">
                            <div className="flex justify-between items-start">
                                <span className="text-[11px] font-dev opacity-50 text-transform-tertiary tracking-tighter">{selectedService.id}</span>
                                <Pill variant={selectedService.status_id === '1' ? 'success' : 'neutral'}>
                                    {selectedService.status_id === '1' ? t.status_active : t.status_inactive}
                                </Pill>
                            </div>
                            <h3 className="font-bold text-lg text-on-surface leading-tight">{selectedService.name}</h3>
                            <div className="pt-2 border-t border-primary/10 grid grid-cols-2 gap-4">
                                <div>
                                    <span className="block text-[10px] text-transform-tertiary opacity-40 font-bold mb-1">{t.label_price}</span>
                                    <span className="text-sm font-dev font-bold">
                                        {selectedService.qty_on_hand.toLocaleString()} VND
                                    </span>
                                </div>
                                <div>
                                    <span className="block text-[10px] text-transform-tertiary opacity-40 font-bold mb-1">{t.label_category}</span>
                                    <span className="text-sm font-bold text-transform-tertiary">{selectedService.category_id}</span>
                                </div>
                            </div>
                        </div>
                    </div>
                    ...
                    <div className="p-8 rounded-xl border border-dashed border-border flex flex-col items-center justify-center text-center space-y-3 opacity-40">
                        ...
                        <p className="text-xs font-medium px-4">{t.msg_selectService}</p>
                    </div>
                )}
