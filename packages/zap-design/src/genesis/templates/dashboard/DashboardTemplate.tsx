...
export default function DashboardTemplate() {
    ...
    return (
        <AppShell>
        <div className="flex h-screen w-full bg-layer-base font-body text-on-surface text-transform-secondary overflow-hidden">
            {/* LEFT SIDEBAR */}
            {sidebarOpen && (
                <aside className="w-[280px] bg-layer-cover border-r border-border flex flex-col flex-shrink-0 transition-all duration-300">
                    ...
                    <div className="flex-1 w-full overflow-y-auto p-4 flex flex-col gap-2">
                        ...
                        <div className="bg-layer-panel h-10 rounded-btn w-full animate-pulse" />
                        <div className="bg-layer-panel h-10 rounded-btn w-full animate-pulse" />
                        <div className="bg-layer-panel h-10 rounded-btn w-full animate-pulse" />
                    </div>
                </aside>
            ...
            {/* MAIN CONTENT AREA */}
            <main className="flex-1 flex flex-col bg-layer-cover min-w-0">
                {/* HEADER */}
                <header className="h-14 border-b border-border flex items-center justify-between px-4 flex-shrink-0 w-full bg-layer-panel z-10">
                    ...
                    <div className="flex items-center gap-4">
                        <div className="relative hidden md:flex items-center">
                            ...
                            <GenesisInput
                                ...
                                className="pl-8 w-[200px] lg:w-[300px] bg-layer-cover border-border"
                            />
                        </div>

                        <GenesisButton visualStyle="ghost" className="hidden md:flex">Upgrade to Pro</GenesisButton>
                        <GenesisButton visualStyle="ghost" className="text-on-surface-variant hover:text-on-surface">
                            <Bell className="size-5" />
                        </GenesisButton>

                        <GenesisButton visualStyle="outline" className="gap-2 border-border shadow-sm">
                ...
                <div className="flex-1 w-full overflow-y-auto p-6 md:p-10">
                    <div className="max-w-6xl mx-auto flex flex-col gap-6">
                        ...
                        <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
                            <Card className="p-6 flex flex-col gap-4 bg-layer-panel">
                        ...
                        <Card className="p-6 flex flex-col gap-6 bg-layer-panel">
                            <div className="flex justify-between items-center">
                                <h2 className="font-display text-headlineMedium text-on-surface text-transform-primary">Recent Swarm Activity</h2>
                                <GenesisButton visualStyle="solid" className="bg-primary text-on-primary">View All Reports</GenesisButton>
                            </div>
