    ...
    return (
        <div className="flex items-center gap-4 p-3 rounded-[16px] bg-surface-container hover:bg-surface-container-high transition-colors relative">
            ...
            <div className="flex flex-col pr-8">
                ...
                {description && <span className="text-[11px] text-on-surface-variant leading-tight mt-0.5">{description}</span>}
            </div>
            {!isPrimary && color && (
...
const ThemeGrid = ({ scheme, isDark, title }: { scheme: any, isDark: boolean, title: string }) => {
    ...
    return (
        <section className="w-full rounded-2xl overflow-hidden border shadow-sm bg-layer-panel" style={{ borderColor: outlineColor }}>
            {/* ── Cover Card Header ─────────────────────────────────────── */}
            <div
                className="flex items-center justify-between px-6 py-4 border-b bg-layer-dialog"
...
const FixedGrid = ({ palettes, scheme }: { palettes: Record<string, any>, scheme: any }) => {
    ...
    return (
        <section>
            ...
            <div className="rounded-xl p-8 border shadow-sm transition-colors w-full bg-layer-panel" style={{ borderColor: outline }}>
...
const IOSScreenContent = ({ s }: { s: Record<string, string> }) => (
    <div className="flex-1 overflow-y-auto no-scrollbar flex flex-col px-6 pt-2">
        ...
        <h2 className="text-[30px] leading-tight text-center mb-5 font-display" style={{ color: s.primary }}>
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar p-6 flex flex-col gap-5" style={{ backgroundColor: s.surfaceContainerLow }}>
            ...
            <div className="flex gap-3">
                {['Water All', 'Schedule', 'Add Plant'].map((label, i) => (
                    <button
                        key={i}
                        className="flex-1 rounded-full py-2.5 text-[11px] font-bold uppercase tracking-wider transition-colors"
                        style={{
        ...
        <div className="flex-1 overflow-y-auto no-scrollbar p-8 flex flex-col gap-6" style={{ backgroundColor: s.surfaceContainerLow }}>
            <div className="flex items-center justify-between">
                ...
                <div className="flex gap-2">
                    <button className="rounded-full px-4 py-2 text-[11px] font-bold uppercase tracking-wider" style={{ backgroundColor: s.primary, color: s.onPrimary }}>Add Plant</button>
                </div>
            </div>
                ...
                {WINDOW_CLASSES.map(({ id, label }) => {
                    ...
                    return (
                        <button
                            ...
                            className="px-5 py-2 rounded-full text-[12px] font-bold uppercase tracking-widest transition-all duration-200"
                            style={{
                    ...
                    {([
                        ...
                        return (
                            <button
                                ...
                                className="flex items-center gap-1.5 px-4 py-1.5 rounded-full text-[10px] font-bold uppercase tracking-widest transition-all duration-200"
                                style={{
...
export default function MaterialKolorClone() {
    ...
    React.useEffect(() => {
        const loadInitialSettings = async () => {
            try {
                // Load saved colors
                const colorRes = await fetch('/api/colors/publish?theme=metro');
                if (colorRes.ok) {
...
${generateCssVars(darkScheme)}
    ...
    return (
        <DebugAuditor
            ...
            tier="L1 TOKEN"
            status="Beta"
            filePath="src/app/debug/metro/colors/page.tsx"
                        ...
                        <div className="w-full flex flex-col gap-3">
                            ...
                            >
                                <span 
                                    ...
                                    style={{ fontFamily: activeFont || 'var(--font-display, sans-serif)', textTransform: activeTransform }}
                                >
                    ...
                    <div className="pt-2 pb-8 flex flex-col w-full">
                        ...
                        <InspectorAccordion title="Seed Color" icon="palette" defaultOpen={true}>
                            <div className="space-y-4 pt-1 pb-2">
                                <p className="text-[11px] text-on-surface-variant leading-relaxed">Extract from image or hex.</p>
                                ...
                                <div className="mt-2 p-3.5 rounded-[16px] flex items-center justify-between shadow-sm transition-colors bg-secondary-container">
                                    <div className="flex items-center gap-3">
                                        ...
                                        <div className="flex flex-col">
                                            <span className="text-[15px] font-medium" style={{ color: hexFromArgb((isDarkMode ? darkScheme : lightScheme).onSecondaryContainer) }}>Seed color</span>
                                            <span className="text-[10px] leading-tight opacity-80" style={{ color: hexFromArgb((isDarkMode ? darkScheme : lightScheme).onSecondaryContainer) }}>Material 3 seed generator.</span>
                                        </div>
                                    </div>
                                </div>
                            </div>
                        ...
                        <InspectorAccordion title="Core Overrides" icon="layers">
                            <div className="space-y-4 pt-1 pb-2">
                                <p className="text-[11px] text-on-surface-variant">Override scheme keys.</p>
                        ...
                        {/* 3. Style */}
                        <InspectorAccordion title={`Theme Style: ${SCHEME_VARIANTS.find(v => v.id === schemeVariant)?.label || schemeVariant}`} icon={Sparkles}>
                            <div className="space-y-4 pt-1 pb-2">
        ...
        >
            <Canvas className="transition-all duration-300 origin-center flex flex-col pt-0 min-h-full">
                ...
                <div className="w-full flex-none">
                    <ThemeHeader
                        title={
                            activeTab === 'preview' ? 'Preview Mockup'
                                : 'Color Matrices'
                        }
                        breadcrumb="Zap Design Engine / Foundations"
                        badge={
                            activeTab === 'preview' ? 'Experimental Component Preview'
                                : 'L1: Core Color Map'
                        }
                        tabs={COLOR_TABS}
                        activeTab={activeTab}
                ...
                <div className="flex-1 w-full p-6 md:p-12 flex flex-col items-center">
                    <div className="w-full max-w-[1080px] flex-1 rounded-[32px] border bg-layer-cover border-outline-variant overflow-hidden flex flex-col">
                        <div className="flex-1 w-full p-8 md:p-12 flex flex-col">
                            {/* Page Level Title */}
                            <div className="mb-8 flex flex-col md:flex-row md:items-center justify-between gap-4">
                                ...
                                <div className="hidden md:flex flex-col gap-2">
                                    <div className="flex items-center gap-2">
                                        <Badge className="bg-layer-canvas text-on-surface border-outline-variant font-dev text-[10px] tracking-widest text-transform-tertiary">L1: Canvas (Bg: surface-container-low)</Badge>
                                        <Badge className="bg-layer-cover text-on-surface border-outline-variant font-dev text-[10px] tracking-widest text-transform-tertiary">L2: Cover (Bg: surface)</Badge>
                                    </div>
                                    <div className="flex items-center gap-2">
                                        <Badge className="bg-layer-panel text-on-surface border-outline-variant font-dev text-[10px] tracking-widest text-transform-tertiary">L3: Panel (Bg: surface-container-high)</Badge>
                                        <Badge className="bg-surface-container-highest text-on-surface border-outline-variant font-dev text-[10px] tracking-widest text-transform-tertiary">L4: Elevated (Bg: surface-container-highest)</Badge>
                                    </div>
                                </div>
                            </div>
