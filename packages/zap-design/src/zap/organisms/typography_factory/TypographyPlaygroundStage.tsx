...
const getLayerStyles = (settings?: LayeredStylizationSettings): React.CSSProperties => {
    ...
    if (settings.effects?.enabled) {
        ...
        if (settings.effects.textTransform && settings.effects.textTransform !== 'none') styles.textTransform = settings.effects.textTransform;
...
const FontSelector = ({ value, onChange, label }: { value: string, onChange: (val: string) => void, label: string }) => (
    <label className="flex items-center gap-2 bg-white px-3 py-1 border-[length:var(--card-border-width,2px)] border-brand-midnight rounded-full shadow-[2px_2px_0_0_var(--color-brand-midnight)]">
        <span className="text-labelSmall font-body text-transform-secondary font-bold text-gray-500">{label}:</span>
        <Select
            options={CORE_FONTS.map(f => ({ label: f.name, value: f.value }))}
            value={value}
            onChange={onChange}
            className="text-labelMedium font-body text-transform-secondary text-brand-midnight bg-transparent w-32"
        />
    </label>
);

export const TypographyPlaygroundStage = ({ initialTheme, onUpdateGlobalTheme }: TypographyPlaygroundShellProps) => {
    ...
    const handleComponentPublish = async (id: string) => {
        if (!theme) return;
        const currentData = theme.elements[id];

        console.log(`Publishing typography overrides for component: ${id}`);
        try {
            const existingRes = await fetch(`/api/typography/publish?theme=${theme.id}`);
            const existing = await existingRes.json();
            const payload = existing.data || { theme: theme.id };

            if (!payload.components) payload.components = {};
            
            payload.components[id] = {
                fontFamily: currentData.fontFamily,
                textTransform: currentData.effects?.uppercase ? 'uppercase' : 
                               // eslint-disable-next-line @typescript-eslint/no-explicit-any
                               (currentData.effects as any)?.lowercase ? 'lowercase' : 
                               // eslint-disable-next-line @typescript-eslint/no-explicit-any
                               (currentData.effects as any)?.capitalize ? 'capitalize' : 'none'
            };
            ...
            if (data.success) {
                alert(`${id} typography published to Global ${theme.id.toUpperCase()} Theme!`);
            } else {
                alert("Failed to publish component typography: " + data.error);
            }
        } catch (err) {
            console.error("Error publishing component typography:", err);
            alert("Network error publishing typography.");
        }
    };
    ...
    return (
        <Wrapper identity={{ displayName: `Shell - ${theme.name}`, filePath: "zap/sections/atoms/typography/playground-shell.tsx", type: "Atom/View" }}>
            <div className="relative overflow-hidden transition-all duration-300 border border-border/50 rounded-md bg-card shadow-sm">
                <div className="p-12 space-y-16">
                    ...
                    {activeAtom && theme.elements[activeAtom] && (
                        <DraggableToolbox
                            ...
                            onPublishComponent={() => handleComponentPublish(activeAtom)}
                        />
                    )}
                    ...
                    {theme.decorations?.map(deco => {
                        if (deco.type === 'blob' || deco.type === 'sticker' || deco.type === 'tag') {
                            return <div key={deco.id} className={deco.className} style={deco.style}>{deco.content}</div>;
                        }
                        return null;
                    })}
                    ...
                    {PLAYGROUND_SECTIONS.map((section) => (
                        <section key={section.role} className="space-y-8 relative z-10 bg-white/50 backdrop-blur-sm p-8 border-[length:var(--card-border-width,2px)] border-brand-midnight rounded-card shadow-sm">
                                        ...
                                        <Wrapper key={entry.token.id} identity={{
                                            ...
                                            <div className={`grid grid-cols-12 gap-4 ${entry.token.fontFamilyRole === 'primary' ? 'items-center' : 'items-start'} border-b-[length:var(--card-border-width,2px)] border-brand-midnight/20 pb-4 border-dashed relative`}>
                                                <div className="col-span-3 text-pink-500 pt-1">
                                                    <Text size="iso-100" weight="bold" className="text-transform-secondary tracking-widest">
                                                ...
                                                <div className="col-span-9 transition-transform group cursor-pointer" onClick={(e) => handleAtomClick(entry.token.m3Name, e)}>
                                                    <Tag
                                                        ...
                                                        style={{ ...getLayerStyles(settings), ...(settings ? {} : { color: 'black' }) }}
                                                    >
                                                        {entry.sampleText}
                                                    </Tag>
                                                </div>
                    ...
                    {/* COMPONENT-DRIVEN SECTION */}
                    <section className="space-y-8 relative z-10 bg-white/50 backdrop-blur-sm p-8 border-[length:var(--card-border-width,2px)] border-brand-midnight rounded-card shadow-sm">
                        <div className="flex items-center justify-between">
                            <SectionHeader
                                number="07"
                                title="UI Components"
                                icon="widgets"
                                description="Component-level typography overrides (e.g. Buttons, Badges)."
                                id="playground-components"
                            />
                        </div>

                        <div className="space-y-6 relative z-10">
                            <Wrapper identity={{ displayName: `Button Typography`, filePath: "zap/sections/atoms/typography/playground-shell.tsx", type: "Wrapped Snippet", codeSnippet: "<Button>Sample Button</Button>" }}>
                                <div className="grid grid-cols-12 gap-4 items-center border-b-[length:var(--card-border-width,2px)] border-brand-midnight/20 pb-4 border-dashed relative">
                                    <div className="col-span-3 text-pink-500 pt-1">
                                        <Text size="iso-100" weight="bold" className="text-transform-secondary tracking-widest">
                                            Button Component Override
                                        </Text>
                                    </div>
                                    <div className="col-span-9 transition-transform group cursor-pointer" onClick={(e) => handleAtomClick('button', e)}>
                                        <Button
                                            variant="primary"
                                            className="relative z-10 transition-all duration-200"
                                            style={getLayerStyles(theme.elements?.['button'])}
                                        >
                            ...
                            <Wrapper identity={{ displayName: `Checkbox Typography`, filePath: "zap/sections/atoms/typography/playground-shell.tsx", type: "Wrapped Snippet", codeSnippet: "<Checkbox id='playground-cb' /> <label htmlFor='playground-cb'>Publish Action Label</label>" }}>
                                <div className="grid grid-cols-12 gap-4 items-center border-b-[length:var(--card-border-width,2px)] border-brand-midnight/20 pb-4 border-dashed relative">
                                    <div className="col-span-3 text-pink-500 pt-1">
                                        <Text size="iso-100" weight="bold" className="text-transform-secondary tracking-widest">
                                            Checkbox Component Override
                                        </Text>
                                    </div>
                                    <div className="col-span-9 transition-transform group cursor-pointer" onClick={(e) => handleAtomClick('checkbox', e)}>
                                        <div 
                                            className="relative z-10 transition-all duration-200 flex items-center gap-3 w-fit"
                                            style={getLayerStyles(theme.elements?.['checkbox'])}
                                        >
                                            <Checkbox id="playground-checkbox-preview" />
                                            <label 
                                                htmlFor="playground-checkbox-preview"
