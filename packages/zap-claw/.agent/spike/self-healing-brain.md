...
This agent is equipped with a self-healing autonomic loop. When a critical failure occurs (e.g., API 403, Syntax Error, Token Exhaustion), the agent **MUST NOT** immediately crash or halt execution. It must attempt to diagnose and self-patch the issue before escalating to `Zeus (Tom)`.
...
3. **Trigger RCA (Root Cause Analysis):** The agent will analyze the stack trace using the Deep Brain (`Gemini 3.1 (Deep Brain - SOTA)`).
...
- It must generate a concise Post-Mortem summarizing what failed and what 3 patches it attempted, delivering this directly to `Zeus (Tom)`.
