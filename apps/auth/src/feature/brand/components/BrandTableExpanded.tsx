                ...
                <div className="space-y-4 px-7 py-4 border-b border-border">
                  ...
                  <div className="grid grid-cols-2 gap-4 font-dev">
                    <div>
                      ...
                      <p className="text-foreground text-xs font-mono">{brand.slug}</p>
                    </div>
                    <div>
                      <p className="mb-1 text-[length:var(--table-font-size,0.625rem)] font-display font-semibold text-transform-primary tracking-wide text-muted-foreground">
                        Status ID
                      </p>
                      <p className="text-foreground">{brand.status_id}</p>
                    </div>
                  </div>
                </div>
              </motion.div>
    ...
    >
      ...
      <div className="space-y-3">
        <p className="text-[length:var(--table-font-size,0.625rem)] font-display font-semibold text-transform-primary tracking-wide text-muted-foreground">
          {t('table_premium', 'Premium Status')}
        </p>
        <div className="space-y-2">
          {['Premium', 'Standard'].map((status) => {
...
export function BrandTableExpanded({
  ...
  const [internalFilters, setInternalFilters] = useState<BrandFilters>({ status: [] });
  ...
  const filteredBrands = useMemo(() => {
    return brands.filter((brand) => {
      ...
      const matchSearch = brand.name.toLowerCase().includes(lowerQuery) || brand.slug.toLowerCase().includes(lowerQuery);
      const brandStatus = brand.is_premium ? 'Premium' : 'Standard';
      const matchStatus =
        filters.status.length === 0 || filters.status.includes(brandStatus);
        ...
        <div className="flex items-center h-8">
          {hasActiveFilter && (
            <span className="text-sm font-medium text-muted-foreground font-body text-transform-secondary">
              {filteredBrands.length} of {brands.length} records matched criteria.
            </span>
          )}
        </div>
        ...
        <div className="flex items-center gap-3 w-full md:w-auto">
          <div className="relative flex-1 md:w-80">
            <Input
              variant="filled"
              leadingIcon="search"
              placeholder={t('search_placeholder', 'Search brands by name or slug...')}
              value={searchQuery}
              onChange={(event) => {
