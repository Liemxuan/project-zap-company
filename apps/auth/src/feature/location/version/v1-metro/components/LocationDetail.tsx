...
export function LocationDetail({ location: initialLocation, locationId, onCancel }: LocationDetailProps) {
    ...
    return (
        <div className="fixed inset-0 z-[100] flex flex-col bg-layer-cover overflow-hidden">
            ...
            {/* ── Sticky Action Bar (Down/Bottom) ── */}
            <div className="border-t border-outline-variant/40 bg-layer-base p-4 flex justify-end gap-3 shrink-0 z-20 shadow-[0_-4px_12px_rgba(0,0,0,0.03)]">
                <Button 
                    variant="ghost" 
                    size="sm" 
                    onClick={onCancel}
                    className="font-display text-transform-primary text-xs h-9 px-6 rounded-lg"
                >
                    Cancel
                </Button>
                <Button 
                    ...
                    onClick={handleSave}
                    disabled={saving}
                    className="font-display text-transform-primary text-xs h-9 px-8 rounded-lg shadow-sm"
                >
