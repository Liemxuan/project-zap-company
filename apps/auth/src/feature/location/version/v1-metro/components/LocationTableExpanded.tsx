...
export function LocationTableExpanded({
  ...
  const [internalFilters, setInternalFilters] = useState<LocationFilters>({ is_active: [], location_type_text: [] });
  ...
  const columns = useMemo<ColumnDef<Location>[]>(() => [
    {
      id: "select",
      header: ({ table }) => (
        <div className="w-12 px-7 flex items-center justify-center">
          <Checkbox
            checked={
      ...
      cell: ({ row }) => (
        <div className="w-12 px-7 flex items-center justify-center" onClick={e => e.stopPropagation()}>
          <Checkbox
    ...
    {
      accessorKey: 'id',
      header: () => <div className="w-40 text-left font-mono text-[10px] tracking-widest text-muted-foreground uppercase">{t('table_column_id', 'ID')}</div>,
      cell: ({ row }) => (
        <div className="w-40 py-2.5 font-dev text-[10px] text-muted-foreground truncate uppercase opacity-70">
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="w-32 text-left py-2.5">
            <Pill
              variant={variant}
              className="whitespace-nowrap w-fit uppercase"
            >
    ...
    },
    {
      id: "actions",
      header: ({ table }) => (
        <div className="w-32 pr-7 flex items-center justify-end">
          <ColumnPicker table={table} t={t} />
        </div>
      ),
      cell: ({ row }) => (
        <div className="w-32 pr-7 py-2.5 text-right flex items-center justify-end" onClick={e => e.stopPropagation()}>
          <DropdownMenu>
