...
import { ModifierGroupTableExpanded, type ModifierGroupFilters } from '@/feature/modifier-group/components/ModifierGroupTableExpanded';
import { useModifierGroups } from '@/feature/modifier-group/hooks/use-modifier-groups';
import { loadTranslations, t as translate } from '@/lib/i18n';
import type { SupportedLang } from '@/const';

// ── Page ─────────────────────────────────────────────────────────────────────

export default function AuthModifierGroupsPage() {
    ...
    useEffect(() => {
        loadTranslations(lang, 'modifier_group')
    ...
    const { modifierGroups, loading, totalRecords, totalPages } = useModifierGroups(currentPage, pageSize);
    ...
    const inspectorContent = (
        <Inspector title={t('inspector_title', 'MODIFIER GROUPS LAB')} width={320}>
    ...
    return (
        <div className="flex h-screen w-full bg-layer-canvas overflow-hidden font-sans">
            ...
            <div className="flex-1 flex flex-col min-w-0 overflow-hidden relative">
                <ThemeHeader
                    title={t('title', 'modifier group assembly')}
                    breadcrumb={`zap auth / ${theme} / modifier group vault`}
