...
export default function AuthUnitsPage() {
    ...
    const { theme: appTheme, inspectorState, setInspectorState, setTheme } = useTheme();
        ...
        <div className="flex h-full w-full bg-layer-base overflow-hidden font-body border border-border">
            ...
            <aside className="w-[240px] flex-shrink-0 border-r border-border bg-layer-panel hidden md:flex flex-col z-10 shadow-sm relative">
                ...
                <nav className="flex-1 py-4 px-3 space-y-1 overflow-y-auto">
                    {[
                        { icon: 'dashboard', label: t('nav_overview', 'overview') },
                        { icon: 'list_alt', label: t('nav_logs', 'system logs') },
                        { icon: 'inventory-2', label: t('nav_units', 'unit list'), active: true },
                        { icon: 'settings', label: t('nav_config', 'configuration') },
                    ].map((item) => (
                        <div
                            key={item.label}
        ...
        >
            <div className="w-full flex-1 flex items-center justify-center pt-8">
                <CanvasDesktop
                    ...
                    fullScreenHref={`/auth/${activeTheme}/units?fullscreen=true`}
                >
                    {layoutContent}
                </CanvasDesktop>
            </div>
