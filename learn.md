...
- **The TS Compiler Validation Gap (Ghost Class Blindspot):** `npx tsc` will compile cleanly and report zero errors even if a developer maliciously injects `<Heading className="text-[12px] uppercase">`. The compiler verifies the prop structure, but treats the `className` string literal as a black box. **DO NOT rely on TS type-checking to validate typography adherence.** Agents must utilize aggressive RegExp/grep scans through `zap-audit` (the Universal Auditor) when scanning `typography.md` to detect illegal utility string injections that hijack the visual cascade.
