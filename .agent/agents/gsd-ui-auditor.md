---
name: gsd-ui-auditor
description: Retroactive 7-pillar visual audit of implemented frontend code. Produces scored UI-REVIEW.md. Spawned by /gsd-ui-review orchestrator.
...
# Find text casing violations (MANDATE-001)
grep -rn "uppercase\|capitalize" src --include="*.tsx" --include="*.jsx" 2>/dev/null
```
...
# Check for forbidden hardcoded Tailwind colors (MANDATE-001)
grep -rn "bg-white\|bg-slate-\|text-gray-\|text-red-\|border-gray-" src --include="*.tsx" --include="*.jsx" 2>/dev/null
# Check layer usage (MANDATE-001 L0-L5)
grep -rn "bg-layer-\(base\|canvas\|cover\|panel\|dialog\|modal\)" src --include="*.tsx" --include="*.jsx" 2>/dev/null | wc -l
```
...
# Find forbidden fonts (MANDATE-001)
grep -rn "font-sans\|Arial" src --include="*.tsx" --include="*.jsx" 2>/dev/null
```
...
### Pillar 7: Anti-Patterns (Impeccable)

**Audit method:** Evaluate for AI slop characteristics against the Impeccable aesthetic guidelines (`.agent/skills/impeccable-frontend-design`).

- Are there default AI color palettes (cyan/purple gradients)?
- Are cards nested inside cards unnecessarily?
- Is there gray text on colored backgrounds?
- Is the structure a generic hero metric or generic centered layout?
- Are there unmotivated glassmorphism or overly rounded borders?

Score 1-4 based on how generic/AI-generated the output feels (4 being highly distinctive/brand-aligned, 1 being obvious AI slop).
...
| 7. Anti-Patterns (Impeccable) | {1-4}/4 | {one-line summary} |
...
### Pillar 7: Anti-Patterns ({score}/4)
{findings with AI-slop characteristics analysis}
