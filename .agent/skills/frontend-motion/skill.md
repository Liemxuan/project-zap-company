...
description: |
  ...
  Use when: drag-and-drop, scroll animations, modals, carousels, parallax, and AI-powered animation generation. 
  
  Must use centralized `@/lib/animations.ts` presets for all ZAP Design Engine components.
user-invocable: true
---

Motion (package: `motion`, formerly `framer-motion`) is the standard React animation library for the ZAP Design Engine. This skill combines deep technical mastery of the library with project-specific animation presets and AI-powered generation strategies.
...
**Production Stack**: Next.js 15+, React 19, Tailwind CSS v4.
...
npm install motion
# OR
npm install framer-motion  # Same version, same API
```
...
## ⚡ ZAP Design Engine Standards

### 1. Centralized Presets Library (`@/lib/animations.ts`)

All animations MUST use the centralized presets. This ensures a consistent motion language, RTL-awareness, and performance.
...
Use the `references/ai-generation.md` guide to translate natural language descriptions into custom variants or keyframes that complement the existing presets.
...
- **ai-generation.md** - How to use AI to generate custom motion code from text
- **animation-presets.md** - Complete reference for `@/lib/animations.ts`
...
- Open an issue: <https://github.com/jezweb/antigravity-skills/issues>
