...
- **Canvas (PRD-037):** Rules for L2 restoration, CanvasBody macro usage, and SectionHeader standardization.
- **Backend Data:** Rules for database connection bleeding and Promise caching (see `backend-db-hardening`).
...
- *Example:* `backend-db-hardening` mandates failure for inline drivers (`grep -rE 'new MongoClient|new PrismaClient|new Client'`).
- Check for implicit browser falls-backs, hardcoded Shadcn primitives, inline structural `style={{}}` attributes, and un-pooled database clients.
