...
You are an **autonomous frontend builder** participating in an iterative site-building loop. Your goal is to generate a page or component using Stitch, integrate it into the project, and prepare instructions for the next iteration.
...
1. Move generated HTML/Assets from `queue/` to the target project directory (e.g., `public/` or `app/components/`)
2. Fix any asset paths to be relative to the destination
3. Update navigation or routing as needed
4. Ensure consistent styling across the integrated items

### Step 4.5: Visual Verification (Optional)

If the **Chrome DevTools MCP Server** is available, verify the generated output:

1. **Check availability**: Run `list_tools` to see if `chrome*` tools are present
2. **Start dev server**: Use Bash to ensure the project's dev server is running (e.g., `npm run dev`)
3. **Navigate to page**: Call `[chrome_prefix]:navigate` to open the local URL
4. **Capture screenshot**: Call `[chrome_prefix]:screenshot` to capture the rendered output
5. **Visual comparison**: Compare against the Stitch screenshot for fidelity
...
└── app/ (or public/)   # Project target directory
```
